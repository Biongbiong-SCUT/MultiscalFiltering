...
void CalculationThread::initAlgorithm(DataManager *_data_manager, ParameterSet *_parameter_set)
{
    ...
    case kBilateralMeshDenoising:
		...
		mesh_denoise_base_ = new MSFiltering(_data_manager, _parameter_set);
		break;
    ...
    case kGuidedMeshNormalFiltering:
        ...
        break;
    case kRollingGuidedNormalFiltering:
        mesh_denoise_base_ = new RollingGuidedNormalFiltering(_data_manager, _parameter_set);
        break;
	case kMyBilateralFiltering:
		mesh_denoise_base_ = new SDMeshDenoisying(_data_manager, _parameter_set);
		break;
	case kMSFitering:
		// mesh_denoise_base_ = new MSFiltering(_data_manager, _parameter_set);
		break;
