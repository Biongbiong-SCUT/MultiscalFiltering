...
void MainWindow::CreateActions()
{
    ...
    action_render_normals_ = new QAction(this);
    action_render_normals_->setText("Render Normals");
    action_render_normals_->setStatusTip("Show All normals");
    action_render_normals_->setCheckable(true);
    action_render_normals_->setChecked(false);
    connect(action_render_normals_, SIGNAL(toggled(bool)), opengl_viewer_, SLOT(setDrawNormalStatus(bool)));

    action_render_filtered_normals_ = new QAction(this);
    action_render_filtered_normals_->setText("Render Filtered Normals");
    action_render_filtered_normals_->setStatusTip("Show All Filtred Normal");
    action_render_filtered_normals_->setCheckable(true);
    action_render_filtered_normals_->setChecked(false);
    connect(action_render_filtered_normals_, SIGNAL(toggled(bool)), this, SLOT(ShowFiltereNormal(bool)));
	...
	action_bilateral_mesh_denoising_ = new QAction(tr("Multi-Scale Mesh Filtering"), this);
    ...
    //Rolling Guided Normal
    action_rolling_guided_normal_filtering_ = new QAction(tr("Rolling Guided Normal Filtering"), this);
    action_rolling_guided_normal_filtering_->setStatusTip("Denosie Mesh using Algorithm -- Rolling Guided Normal Filtering.");
    connect(action_rolling_guided_normal_filtering_, SIGNAL(triggered()), this, SLOT(ShowRollingGuidedNormalFilteringWidget()));

	action_my_bilatering_mesh_denoisying_ = new QAction(tr("Static/Dynamic Mesh Filtering"), this);
	action_my_bilatering_mesh_denoisying_->setStatusTip("Denosie Mesh using my Algorithm -- Bilateral Mesh Filtering. ");
	connect(action_my_bilatering_mesh_denoisying_, SIGNAL(triggered()), this, SLOT(ShowMyBilateralFiltering()));
	...
	//Analysis 
	action_compare_with_mesh_ = new QAction(tr("Compare with Original mesh"), this);
	action_compare_with_mesh_->setStatusTip("Compare with Original Mesh");


	action_load_original_mesh_ = new QAction(tr("Load Original mesh"), this);
	action_load_original_mesh_->setStatusTip("Loading Original mesh");
	connect(action_load_original_mesh_, SIGNAL(triggered()), this, SLOT(ImportMeshOriginal()));
...
void MainWindow::CreateMenus()
{
    ...
    menu_algorithms_->addAction(action_rolling_guided_normal_filtering_);
	menu_algorithms_->addAction(action_my_bilatering_mesh_denoisying_);
    menu_algorithms_->setEnabled(false);


	menu_analysis_ = menuBar()->addMenu(tr("Analysis"));
	menu_analysis_->addAction(action_compare_with_mesh_);
	menu_analysis_->addAction(action_load_original_mesh_);
	menu_analysis_->setEnabled(true);
...
void MainWindow::CreateToolBars()
{
    ...
    toolbar_opengl_info_->addAction(action_render_normals_);
    toolbar_opengl_info_->addAction(action_set_background_color_);
    toolbar_opengl_info_->addAction(action_render_filtered_normals_);
...
{
    QString filename = QFileDialog::getOpenFileName(this, tr("Import Mesh"), "../../RNG_model", tr(" *.obj *.off *.ply"));
...
void MainWindow::ShowFiltereNormal(bool _val)
{
    opengl_viewer_->setDrawFilteredNormalsStatus(_val);
    opengl_viewer_->updateGL();
}

void MainWindow::ImportMeshOriginal()
{
	...
	io_thread_->setFileName(filename);
	io_thread_->io_type_ = ioThread::kImportOriginal;
	io_thread_->start();
}
...
void MainWindow::ShowRollingGuidedNormalFilteringWidget()
{
    calculation_thread_->algorithms_type_ = CalculationThread::kRollingGuidedNormalFiltering;
    //qDebug() << " showrollingguidednormalfilteringwidget is call";
    CloseWidget();
    ShowWidget();
}

void MainWindow::ShowMyBilateralFiltering()
{
	calculation_thread_->algorithms_type_ = CalculationThread::kMyBilateralFiltering;
	CloseWidget();
	ShowWidget();
}
...
void MainWindow::needToUpdateGL(bool value)
{
	...
	data_manager_->getRenderValues(values);
    data_manager_->getFilteredNormals(normals);
    opengl_viewer_->setFilteredNormals(normals);
	opengl_viewer_->setValueToShow(values);
    opengl_viewer_->updateGL();
}
